...
tape('connectionsApi(app, connectionManager)', t => {
  t.test('typical usage', t => {
    ...
    const server = app.listen(8080, async () => {
      const connections1 = await get('http://52.231.182.69:8080/v1/connections');

      t.deepEqual(connections1, [], 'GET /v1/connections initially returns the empty array');

      const connection1 = await post('http://52.231.182.69:8080/v1/connections');
      ...
      const connection2 = await get(`http://52.231.182.69:8080/v1/connections/${connection1.id}`);

      t.deepEqual(connection1, connection2, 'GET /v1/connection/$id returns the connection');

      const connections2 = await get('http://52.231.182.69:8080/v1/connections');

      t.deepEqual(connections2, connectionManager.toJSON(), 'GET /v1/connections later returns the updated array');

      const localDescription = await get(`http://52.231.182.69:8080/v1/connections/${connection2.id}/local-description`);
      ...
      const remoteDescription1 = await get(`http://52.231.182.69:8080/v1/connections/${connection2.id}/remote-description`);

      t.deepEqual(remoteDescription1, {}, 'GET /v1/connections/$id/remote-description returns the .remoteDescription');

      const remoteDescription2 = await post(`http://52.231.182.69:8080/v1/connections/${connection2.id}/remote-description`, {
        type: 'answer',
        sdp: 'answer'
      });
      ...
      const remoteDescription3 = await get(`http://52.231.182.69:8080/v1/connections/${connection2.id}/remote-description`);
      ...
      const connection3 = await delete_(`http://52.231.182.69:8080/v1/connections/${connection1.id}`);

      t.equal(connection3.state, 'closed', 'DELETE /v1/connection/$id returns the closed WebRtcConnection');

      const connections3 = await get('http://52.231.182.69:8080/v1/connections');
